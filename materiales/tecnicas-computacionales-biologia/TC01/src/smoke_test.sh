...
required=(bash uname pwd mkdir touch cp mv ls wc sha256sum mktemp)
...
sandbox=$(mktemp -d "${TMPDIR:-/tmp}/tc01-smoke.XXXXXX")
cleanup() {
    rm -rf -- "$sandbox"
}
...
test -f "$sandbox/espacio con espacios/datos/mini.fasta"
test -f "$sandbox/espacio con espacios/resultados/entorno_comprobado.txt"
test "$(wc -l < "$sandbox/espacio con espacios/datos/mini.fasta")" -eq 6

mkdir -p -- "$(dirname -- "$report")"
data_sha256=$(sha256sum "$data_dir/mini.fasta")
