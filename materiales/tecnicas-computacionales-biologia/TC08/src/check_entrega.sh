#!/usr/bin/env bash
# Verificador de entrega de TC-CH08. No puntúa: la nota la pone quien corrige.
...
Wabs="$(cd "$W" && pwd)"
salida=$(bash "$AQUI/comprobar_matriz.sh" "$Wabs" 2>&1) || true
printf '%s' "$salida" | grep -q '^MATRIZ_OK' || { ko "la matriz de transición estimada no cumple:"; printf '%s\n' "$salida" | grep '^FALLO' | sed 's/^/    /'; }
if [ -f "$W/notas/respuestas.tsv" ]; then
  ...
  awk -F'\t' 'NR>1 && tolower($1) ~ /viterbi/' "$W/notas/respuestas.tsv" | grep -q . || ko "falta la fila de la decodificación de Viterbi"
...
grep -rqs "PATH:" "$W/resultados" || ko "resultados/ no recoge ninguna ruta de Viterbi"
grep -rqs "NAIVE_PATH:" "$W/resultados" || ko "resultados/ no recoge la comparación con el clasificador posicional"
...
if [ -n "$TAR" ]; then
  if [ -f "$TAR" ]; then t=$(mktemp -d); trap 'rm -rf "$t"' EXIT
    ...
    base=$(basename "$W"); [ -f "$t/$base/notas/mi_matriz.tsv" ] || ko "el paquete no contiene notas/mi_matriz.tsv bajo $base/"
