#!/usr/bin/env bash
# Verificador de entrega de BC-CH07. No puntúa.
set -uo pipefail
W="${1:-}"; TAR="${2:-}"
AQUI="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
fallos=0; ko() { echo "FALTA: $*"; fallos=$((fallos+1)); }
[ -n "$W" ] && [ -d "$W" ] || { echo "uso: check_entrega.sh DIRECTORIO_ENTREGA [PAQUETE.tar.gz]" >&2; exit 2; }
if [ -f "$W/mi_control_calidad.py" ]; then
  salida=$(python3 "$AQUI/comprobar_calidad.py" "$W/mi_control_calidad.py" 2>&1) || true
  printf '%s' "$salida" | grep -q '^MODULO_OK' || { ko "mi_control_calidad.py no cumple el contrato:"; printf '%s\n' "$salida" | grep -E '^(FALLO|FALTA)' | sed 's/^/    /'; }
else ko "mi_control_calidad.py"; fi
if [ -f "$W/notas/respuestas.tsv" ]; then
  ...
  awk -F'\t' 'NR>1 && tolower($1) ~ /ventana|recorte|amortigua/' "$W/notas/respuestas.tsv" | grep -q . || ko "falta la fila sobre el efecto del tamaño de ventana"
else ko "notas/respuestas.tsv"; fi
[ -d "$W/resultados" ] && [ -n "$(ls -A "$W/resultados" 2>/dev/null)" ] || ko "resultados/ vacía"
grep -rqs "TRIM_LEN\|PHRED_Q" "$W/resultados" || ko "resultados/ no recoge ninguna ejecución del oráculo"
...
if [ -n "$TAR" ]; then
  if [ -f "$TAR" ]; then t=$(mktemp -d); trap 'rm -rf "$t"' EXIT
    ...
    base=$(basename "$W"); [ -f "$t/$base/mi_control_calidad.py" ] || ko "el paquete no contiene el módulo bajo $base/"
