#!/usr/bin/env bash
# Verificador de entrega de BC-CH11. No puntúa: la nota la pone quien corrige.
set -uo pipefail
W="${1:-}"; TAR="${2:-}"
AQUI="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
fallos=0; ko() { echo "FALTA: $*"; fallos=$((fallos+1)); }
[ -n "$W" ] && [ -d "$W" ] || { echo "uso: check_entrega.sh DIRECTORIO_ENTREGA [PAQUETE.tar.gz]" >&2; exit 2; }
if [ -f "$W/mi_procesador_pdb.py" ]; then
  salida=$(python3 "$AQUI/comprobar_procesador.py" "$W/mi_procesador_pdb.py" 2>&1) || true
  printf '%s' "$salida" | grep -q '^PROCESADOR_OK' || { ko "el procesador de estructuras no cumple el contrato:"; printf '%s\n' "$salida" | grep -E '^(FALLO|FALTA)' | sed 's/^/    /'; }
else ko "mi_procesador_pdb.py"; fi
...
if [ -f "$W/notas/respuestas.tsv" ]; then
  ...
  awk -F'\t' 'NR>1 && tolower($1) ~ /temperatura|bfactor|factor/' "$W/notas/respuestas.tsv" | grep -q . || ko "falta la fila sobre los factores de temperatura"
else ko "notas/respuestas.tsv"; fi
[ -d "$W/resultados" ] && [ -n "$(ls -A "$W/resultados" 2>/dev/null)" ] || ko "resultados/ vacía"
grep -rqs "ATOMOS:" "$W/resultados" || ko "resultados/ no recoge ningún informe del procesador"
...
if [ -n "$TAR" ]; then
  if [ -f "$TAR" ]; then t=$(mktemp -d); trap 'rm -rf "$t"' EXIT
    ...
    base=$(basename "$W"); [ -f "$t/$base/mi_procesador_pdb.py" ] || ko "el paquete no contiene el procesador bajo $base/"
