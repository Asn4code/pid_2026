#!/usr/bin/env bash
# Verificador de entrega de BC-CH12. No puntúa: la nota la pone quien corrige.
set -uo pipefail
W="${1:-}"; TAR="${2:-}"
...
if [ -f "$W/mi_prediccion.py" ]; then
  python3 -m py_compile "$W/mi_prediccion.py" 2>/dev/null || ko "mi_prediccion.py no compila"
  salida=$(python3 "$AQUI/comprobar_prediccion.py" "$W/mi_prediccion.py" 2>&1) || true
  printf '%s' "$salida" | grep -q '^PREDICCION_OK' || { ko "mi_prediccion.py no cumple el contrato:"; printf '%s\n' "$salida" | grep -E '^(FALLO|FALTA)' | sed 's/^/    /'; }
else ko "mi_prediccion.py"; fi
...
if [ -f "$W/notas/respuestas.tsv" ]; then
  head -1 "$W/notas/respuestas.tsv" | grep -q $'^par\tprediccion_identidad\tprediccion_zona\tresultado_identidad\tresultado_zona\tcoincide\tjustificacion' || ko "notas/respuestas.tsv: cabecera distinta de la que genera el espacio de trabajo"
  n=$(awk -F'\t' 'NR>1 && NF>=7 && $2!="" && $3!="" && $4!="" && $5!="" && $6!="" && length($7)>10' "$W/notas/respuestas.tsv" | wc -l)
  [ "$n" -ge 4 ] || ko "notas/respuestas.tsv: $n pares documentados con predicción y resultado; se exigen los cuatro del fichero de datos"
...
if [ -n "$TAR" ]; then
  if [ -f "$TAR" ]; then t=$(mktemp -d); trap 'rm -rf "$t"' EXIT
    ...
    base=$(basename "$W"); [ -f "$t/$base/mi_prediccion.py" ] || ko "el paquete no contiene mi_prediccion.py bajo $base/"
